...
Cinco rotas em `src/app/api/jobs/*` fazem o trabalho que só o tempo decide. Todas
...
O Hobby do Vercel aceita **2 crons por projeto, no máximo 1×/dia**. Cinco crons
...
| --- | --- | --- |
| `/api/jobs/diario` | `0 7 * * *` (04h BRT) | fila-fiscal + snapshot-estoque + assinaturas + sincronizar-catalogos, em sequência |
...
individuais, isola falha por job e devolve 200 com `falhas: n` no corpo — 3 de 4
jobs OK não deve marcar o cron como quebrado. As cinco rotas individuais
...
3. Deploy. As cinco rotas já estão prontas — nada mais muda.
