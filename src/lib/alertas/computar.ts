 ...
 *
 * SEM filtro de usuário: devolve tudo que existe no tenant. Quem corta por
 * permissão e por categoria desligada é `filtrarAlertas`.
 *
...
/**
 * Corta os alertas para UMA pessoa: categorias desligadas em Configurações →
 * Notificações, e permissão por categoria.
 *
...
export function filtrarAlertas(
  alerts: AlertItem[],
  tenant: Pick<Tenant, "alertasDesativados">,
  acessos: Acesso[],
): AlertItem[] {
  const off = new Set(tenant.alertasDesativados);
  const visiveis = alerts.filter(
    (a) => !off.has(a.category) && podeVerCategoria(acessos, a.category),
  );
