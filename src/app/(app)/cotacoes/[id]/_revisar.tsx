...
import { CalendarClock, CalendarDays, Check, Send, Store } from "lucide-react";
...
export function RevisarCotacao({
  ...
  /**
   * Voltar sai para a lista de cotações — e grava antes. Sair de uma tela de
   * formulário perdendo o que foi digitado é o jeito mais barato de fazer o
   * operador desconfiar do sistema.
   */
  function voltar() {
    if (!sujo || !editavel) {
    ...
    startTransition(async () => {
      try {
        ...
        // Sair não é a hora de brigar por validação: o que estava gravado
        // continua lá, e o erro reaparece no "Criar cotação".
      }
      router.push("/cotacoes");
    });
  }
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      >
        {/* Título e "Salvar alterações" dividem a barra: o botão aparece e
            some conforme o formulário suja, e uma linha própria só para ele
            fazia o card pular de altura a cada tecla digitada. */}
        <header className="flex flex-wrap items-center justify-between gap-x-4 gap-y-1 border-b border-line px-4 py-2.5">
          <div className="min-w-0">
            <h3
              ...
              className="font-display text-[15px] font-semibold leading-tight text-ink"
            >
              Cotação de compra
            </h3>
        ...
        <div className="flex flex-col gap-3 p-4">
          ...
          >
            <label className="flex flex-col gap-1 sm:col-span-2 lg:col-span-1">
              ...
              />
              {invalidos.titulo && (
                <span id="erro-titulo" className="text-[12px] font-medium text-danger">
                  {invalidos.titulo}
                </span>
              )}
            </label>
            ...
            <div className="flex flex-col gap-1">
              ...
              <div className="flex min-h-[4.5rem] flex-1 items-center justify-between gap-3 rounded-[var(--radius)] border border-line bg-surface-2 px-3 py-2.5">
                <p className="min-w-0 text-[11px] leading-snug text-muted">
                  O fornecedor informa preços diferentes conforme a quantidade comprada.
                </p>
                <Switch
      ...
      <div className="sticky bottom-0 z-20 flex flex-wrap items-center justify-between gap-3 rounded-[var(--radius-lg)] border border-line bg-surface px-4 py-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] shadow-[var(--shadow-float)]">
        <div className="min-w-0">
          {/* O contador muda a cada item adicionado e a cada fornecedor
              convidado, sem que o foco saia de onde está — sem `aria-live`,
              quem usa leitor de tela não fica sabendo. */}
          <p aria-live="polite" className="text-[13px] text-muted">
            <span className="font-mono tabular-nums">{cotacao.itens.length}</span>{" "}
            {cotacao.itens.length === 1 ? "item" : "itens"} ·{" "}
            <span className="font-mono tabular-nums">{cotacao.convites.length}</span>{" "}
            {cotacao.convites.length === 1 ? "fornecedor" : "fornecedores"}
            {totalUnidades > 0 && (
              <span className="hidden sm:inline"> · {fmtQtd(totalUnidades)} unidades</span>
            )}
          </p>
          ...
          {previsto.total > 0 && (
            <p className="text-[12px] text-faint">
              ≈ <span className="font-mono tabular-nums text-ink-2">{fmtMoney(previsto.total)}</span>{" "}
              pelo custo atual
              {previsto.semCusto > 0 &&
                ` · ${previsto.semCusto} ${previsto.semCusto === 1 ? "item sem custo" : "itens sem custo"} de fora`}
            </p>
          )}
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            onClick={voltar}
            disabled={pendente}
            className="cursor-pointer rounded-full border border-line px-4 py-2 text-sm font-medium text-ink transition-colors hover:bg-surface-2 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {/* "Voltar" sugeria passo anterior; não há mais passos — o botão
                sai da tela, e o rótulo diz para onde. */}
