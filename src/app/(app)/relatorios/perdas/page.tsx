...
export default async function RelatorioPerdas({ searchParams }: { searchParams: Promise<Record<string, string>> }) {
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <KpiCard label="Perda no período" value={brl(d.atual.total)} delta={variacao(d.atual.total, d.anterior.total)} goodWhen="down" destaque />
      ...
      <ChartCard title="Onde mais se perde" subtitle="Custo da perda por produto">
        {d.atual.itens.length === 0 ? (
          ...
          <BarList tone="danger" items={d.atual.itens.slice(0, 10).map((p) => ({ label: p.nome, value: p.custo, sub: p.sku, display: brl(p.custo) }))} />
        )}
      </ChartCard>

      <ChartCard title="Perdas detalhadas" subtitle={periodo.label}>
        {d.atual.itens.length === 0 ? <ChartEmpty mensagem="Sem perdas no período." /> : <TabelaPerdas linhas={d.atual.itens} />}
      </ChartCard>
    </div>
  );
}
