...
export default async function RelatorioPagamentos({
  searchParams,
}: {
  ...
  const d = await withTenant(ctx, async () => {
    ...
    const [mix, fechamentos] = await Promise.all([
      mixPagamento(range, siteId),
      fechamentosCaixa(range, siteId),
    ]);
  ...
  return (
    <RelatorioShell titulo="Pagamentos e caixa" exportTipo="pagamentos">
