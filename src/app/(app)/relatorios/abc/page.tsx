...
export default async function RelatorioABC({ searchParams }: { searchParams: Promise<Record<string, string>> }) {
  const ctx = await requireActiveTenant();
  const periodo = resolvePeriodo(await searchParams);
  const range: Range = { inicio: periodo.inicio, fim: periodo.fim };

  const itens = await withTenant(ctx, async () => curvaABC(range, await getActiveSiteId()));
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <KpiCard label="Classe A" value={String(contagem.A)} hint={total > 0 ? `${Math.round((fatA / total) * 100)}% do faturamento` : undefined} destaque />
      ...
      <ChartCard title="Curva ABC" subtitle="Concentração do faturamento — barras por classe, linha = % acumulado">
        {itens.length === 0 ? <ChartEmpty /> : <ParetoChart itens={itens.map((i) => ({ label: i.nome, value: i.receita, acumuladoPct: i.acumuladoPct, classe: i.classe }))} />}
      </ChartCard>
