...
import { Plus, Trash2, PackageSearch, Check, X } from "lucide-react";
import { cn } from "@/lib/utils";
import { EstadoVazio, fmtQtd } from "../../_catalogo/ui";
import type { CotacaoDetalhe, ProdutoOpcao } from "../_types";
import { adicionarItemAction, editarItemAction, removerItemAction } from "../actions";
...
// se cota algo que ainda não está cadastrado.
    ...
    <div className="flex flex-col gap-4">
      ...
      {cotacao.itens.length === 0 ? (
        <EstadoVazio
          ...
          descricao="Adicione o que você quer cotar. É essa lista que o fornecedor vai receber."
        />
      ) : (
                  ...
                  {editavel && (
                    <div className="flex shrink-0 items-center gap-1">
                      <button
                        ...
                        className="rounded-full px-2.5 py-1 text-[12px] font-medium text-muted transition-colors hover:bg-surface-2 hover:text-ink"
                      >
                        Editar
                      </button>
                      <button
              ...
              {sugestoes.map((p) => (
                <li key={p.id}>
                  <button
                    ...
                    className="flex w-full items-center justify-between gap-2 px-3 py-2 text-left text-sm text-ink transition-colors hover:bg-brand-soft"
                  >
