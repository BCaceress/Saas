...
/** Notas importadas que ainda não viraram entrada — a fila do recebimento. */
...
export async function listarNotasPendentes(): Promise<NotaPendente[]> {
  const notas = await db.fiscalInbound.findMany({
    where: { status: { in: ["PENDENTE", "CONCILIADO"] } },
    select: {
      id: true,
      numero: true,
      serie: true,
      ...
      conciliacao: { select: { status: true, resolucao: true } },
    },
  ...
  return notas.map((n) => ({
    ...
    numero: n.numero,
    serie: n.serie,
    fornecedor: n.emitRazaoSocial,
    dataEmissao: n.dataEmissao.toISOString(),
    valorTotal: Number(n.valorTotal),
    itens: n._count.items,
    pedidoNumero: n.purchaseOrder?.numero ?? null,
    divergencias: n.conciliacao.filter((c) => c.status !== "OK" && !c.resolucao).length,
    status: n.status,
  }));
}
